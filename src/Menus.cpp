...
Menu SettingsMenuInit()
{
	...
	SDL_Color TBStates[3]{ {100, 100, 100, 100}, {196, 22, 22, 255}, {22, 196, 22, 255} };
	self.textButtonsCount = 2;
	self.textButtons = (TextButton*)malloc(sizeof(TextButton) * self.textButtonsCount);
	self.textButtons[SM_SAVE] = TextButtonInit({ winW / 2 - 600, winH / 2 + 250, 500, 100 }, "SAVE", font, TBStates);
	self.textButtons[SM_BACK] = TextButtonInit({ winW / 2 + 100, winH / 2 + 250, 500, 100 }, "BACK", font, TBStates);
	TTF_CloseFont(font);
	...
	self.switchButtons[0] = SwitchButtonInit({ winW / 2 - 400, winH / 2 - 250, 45 ,45 }, "Fullscreen", font, SBStates);
	TTF_CloseFont(font);
